...
main (int argc, char *argv[])
{
  ...
  // Do not set queue size for n0 and n1 link (use default queue size)
  NetDeviceContainer devices;
  devices = pointToPoint.Install (n0n1);

  // Set up the link between node1 and node2 with a specific queue size to create a bottleneck
  NodeContainer n1n2;
  n1n2.Add (n0n1.Get (1));
  n1n2.Create (1);
  ...
  */
  ...
  // PointToPointHelper pointToPoint2;
  // pointToPoint2.SetDeviceAttribute ("DataRate", StringValue ("10Mbps"));  // Change bandwidth to 10Mbps
  // pointToPoint2.SetChannelAttribute ("Delay", StringValue ("100ms"));    // Change delay to 100ms

  // // Set queue size for N1 and N2 link to make it a bottleneck (kept the same)
  // pointToPoint2.SetQueue ("ns3::DropTailQueue", "MaxSize", StringValue (queuesize));
  // NetDeviceContainer devices2;
  // devices2 = pointToPoint2.Install (n1n2);
  // Set error model for the link between node0 and node1
  ...
  // Setup MyApp (sender application) on node0 (N0)
